...
task expand_file_contents{
  ...
  $build_template_files = get_file_names(get-childitem -path $build_dir -filter "project_name.ps1.template")

  $product_template_files = get_file_names(get-childitem -path $product_dir -recurse -filter "*.template")


  expand_all_template_files $base_template_files $local_settings
  expand_all_template_files $build_template_files $local_settings
  ...
  $build_template_files | foreach-object {remove-item -path $_ -force -ErrorAction SilentlyContinue}
